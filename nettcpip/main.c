...
/* @brief   The "/serv/nettcpip" server
 *
 * Work-In-Progress (skeleton code)
 *
 * Handles the socket related messages sent from the kernel for the TCP/IP protocol.
 * The intention is to use the LwIP sources as a library that this server will wrap
 * around in order to provide the TCP/IP stack for CheviotOS. This is similar to how
 * Minix uses LwIP.
 *
 * This will then send messages to the Genet Ethernet driver for the Raspberry Pi 4.
 */
void main(int argc, char *argv[])
{
  ...
  log_info("nettcpip server started");
